  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${baseURL}${searchByTitle ? querySearchURL : idSearchURL}`
      );
      setSearchResults(response.data.results);
      setLoading(false);
    } catch (err) {
      console.log(
        "🚀 ~ file: PageSearch.jsx ~ line 39 ~ handleSubmit ~ err",
        err
      );
    }
  };
            ...
            <Form onSubmit={handleSubmit}>
              ...
              {searchByTitle !== null && (
                <div className="d-grid mx-auto col-10">
                  <MDBBtn rounded className="mb-3" tag="input" type="submit" />
                </div>
              )}
            </Form>
          </Card>
              ...
              <Row xs={1} md={3} className="g-4 justify-content-center">
                {!loading &&
                  searchResults.map((show) => (
                    <Link
                      to="/show-details"
                      onClick={() => props.setShowID(show.id)}
                    >
                      <Card
                        style={{ width: "250px" }}
                        className="me-4"
